...
![[region_and_az.png]]
*Illustration of how regions and AZ is set up. (above)*
...
- Each subnet corresponds to one AZ
...
1.  Choose an Amazon Machine Image (template to be used to create the instance that you want to create)
	...
	- EBS (Elastic Block Storage) Backed AMI or an Instance Store AMI (Amazon Machine Images)
		...
		**EBS Backed v/s Instance Store**
		- Instance Store —The storage is part of the physical server. Sothere is less latency since it is physically attached.
		- EBS Backed — Network attached storage.
		- Instance Store — Temporary storage so when the instance is stopped or terminated , the data is deleted.

> Latency -  the time taken for data to transfer from source to destination.
2. When you have chosen your desired AMI; you must specify the instance type.
	Different kinds of instance types:
	1. T2 - Burstable performance
...
8. Create or choose an existing key pair.
...
When connecting to the created instance, you would have to download a file provided by the AWS to be able to connect to the EC2 instance and then configure the password to the instance using the keypair that you have (either generated or created, depends on the choice you made during the creation of the instance).
...
3. Recovery - So if one instance fails, you can assign the elastic IP to another instance.
---
...
2. Rules are also checked by aws based on their numbering/name. From least to the greatest value.
...
To change the NACL, navigate down the side menu on the left and then find **Network ACLs**. From there you can make modification on your current and existing network ACLs.
...
- For Example: Web server - Only allow traffic to port 8o and 443. Or 3389 for RDP. Port 22 for SSH. Here, you can define what ports and what's IP can be allowed access to your instances.
...
the response traffic for that request is allowed to flow in regardless of inbound security group rule. In Network ACL you have to look at the inbound and the outbound rules. But for the security groups, if you allow the inbound, then the outbound will be automatically allowed. 
...
- this is a way to add additional volumes on your instances. (Storage for EC2 instances)
...
To work with Elastic block storage, go to the EC2 dashboard and navigate to "Volumes" under **Elastic Block store** on the left sidebar menu of the AWS management console.
...
You can also create a snapshot of your EBS volumes. EBS snapshots are a point-in-time copy of the volume. This way you can copy the snapshots of your volumes to other regions to make them available for other instances. If the volume you choose is encrypted, then the snapshot of that volume will also be encrypted and the same in the other case. 

---
# Public and Private Subnets

- **Public Subnet** - subnet that is connected to the internet. So resources can be accessed from the internet which are in this subnet. This is ideal for hosting web servers.
- **Private subnets** do not have a connection to the internet. This is ideal for hosting database servers.
- The private subnet can be connected to the internet via NAT *(Network Address Translation)* instance or a NAT gateway to download updates.

![[public_and_private_subnet_illustration.png]]
*Illustration of how public and private subnets work along with route tables. (above)*

Route tables actually decide on what packets can be routed between subnets.

>[!Note:]
>When you create a VPC from scratch, you don't get an internet gateway. You need to create an internet gateway separately and attach it to the VPC.

You can have route tables separate from each subnet. That is why when you define a private and public subnet, they will have separate route tables. The main route table which is associated with the VPC will be associated with the subnet.

![[private_subnet_with_NAT_illustration.png]]
*Private subnet connecting to the internet via NAT instaance. (above)*

Connecting a private subnet to the internet can be done by first creating a NAT EC2 instance. It is a special AMI (Amazon Machine Images) that is available in AWS. Then in your private subnet you have to add another rule *(see NAT on the route table of the illustration above)*. The NAT instance will go to the internet, to fetch the data and then return it back to the private subnet. And there is no way that traffic will directly go to the private subnet from the internet, it has to pass through the NAT instance.

| Private Subnet         | Public Subnet    |
|------------------------|------------------|
| Host databases servers | Host web servers |

---
# Creating a VPC (Virtual Private Cloud)

### Options for creating a Virtual Private Cloud
1. VPC with a Single public subnet
2. VPC with a Public and Private Subnet (NAT)
3. VPC with a Public and Private Subnet and Hardware VPN Access. This is for connecting your subnets to your own private data center.
4. VPC with a Private Subnet and Hardware VPN Access 

